...
# take care of all the scriptingand setup for tensorflow
...
tee /root/tensorflow/neural_network.ipynb << EOF
{
 "cells": [
  {
   "cell_type": "markdown",
...
EOF
# run a program to make our Jupyter notebook cooler (by getting rid of the output samples until the user generates them)
...
tee /root/tensorflow/notebooksetup.py << EOF
import nbformat

#load the notbook
with open('/root/tensorflow/neural_network.ipynb','r', encoding='utf-8') as f:
...
with open('/root/tensorflow/neural_network.ipynb','w', encoding='utf-8') as f:
    nbformat.write(notebook, f)
EOF
conda run -n tf python /root/tensorflow/notebooksetup.py
